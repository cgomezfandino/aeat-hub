...
Saca un asiento del libro **registrando el motivo** (no-es-factura · calidad-datos · falta-informacion · mal-procesamiento · otro, o texto libre): fuera de totales, vistas y Excel, el fichero pasa a `archivo/…/rechazado/`, y `aeat-hub doctor` agrega los motivos para aprender del pipeline. `aeat-hub reabrir` lo devuelve a revisión. En el tablero, el botón ✕ (o arrastrar a Rechazadas) abre el mismo diálogo con los motivos.

```bash
uv run aeat-hub rechazar 9 --motivo no-es-factura
```
